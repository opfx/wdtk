...
import { Arguments, Option, OptionType, Value } from './types';

export class ParseArgumentException extends CoreException {
  constructor(
 ...
 */
export function parseArguments(args: string[], options: Option[] | null, log?: Logger): Arguments {
  ...
  for (let argIndex = 0; argIndex < args.length; argIndex++) {
    ...
    if (arg.startsWith('--')) {
      consumedNextArg = assignOption(arg, args[argIndex + 1], state);
      ...
      for (let i = 1; i < arg.length; i++) {
        ...
        if (arg[i + 1] == '=') {
          ...
          consumedNextArg = assignOption(f, args[argIndex + 1], state);
          break;
        }
        ...
        if (i == arg.length - 1) {
          ...
          const maybeOption = getOptionFromName(flag, options);
          if (maybeOption) {
            const v = coerce(undefined, maybeOption, parsedOptions[maybeOption.name]);
            if (v !== undefined) {
    ...
    for (let i = 0; i < positionals.length; ) {
      ...
      for (const option of options) {
        ...
        if (option.positional === pos) {
          const coercedValue = coerce(positionals[i], option, parsedOptions[option.name]);
...
export function parseFreeFormArguments(args: string[]): Arguments {
  ...
  for (let arg = args.shift(); arg !== undefined; arg = args.shift()) {
    ...
    if (arg.startsWith('--')) {
      ...
      const v = coerce(value, null, parsedOptions[name]);
      if (v !== undefined) {
...
function coerceType(str: string | undefined, type: OptionType, v?: Value): Value | undefined {
  switch (type) {
    case OptionType.Any:
      if (Array.isArray(v)) {
        return v.concat(str || '');
      ...
      return coerceType(str, OptionType.Boolean, v) !== undefined
        ? coerceType(str, OptionType.Boolean, v)
        : coerceType(str, OptionType.Number, v) !== undefined
        ? coerceType(str, OptionType.Number, v)
        : coerceType(str, OptionType.String, v);
...
function coerce(str: string | undefined, o: Option | null, v?: Value): Value | undefined {
  if (!o) {
    return coerceType(str, OptionType.Any, v);
  } else {
    ...
    for (const type of types) {
      const maybeResult = coerceType(str, type, v);
...
function getOptionFromName(name: string, options: Option[]): Option | undefined {
...
function removeLeadingDashes(key: string): string {
...
function assignOption(
  ...
  if (i == -1) {
    if (key.startsWith('no')) {
      ...
      const maybeOption = getOptionFromName(strings.camelize(key.substr(from)), options);
    ...
    if (option === null) {
      ...
      const maybeOption = getOptionFromName(key, options);
      if (maybeOption) {
        value = nextArg;
        let shouldShift = true;

        if (value && value.startsWith('-') && coerce(undefined, maybeOption) !== undefined) {
          // Verify if not having a value results in a correct parse, if so don't shift.
          shouldShift = false;
        }

        // Only absorb it if it leads to a better value.
        if (shouldShift && coerce(value, maybeOption) !== undefined) {
    ...
    option = getOptionFromName(removeLeadingDashes(key), options) || null;
    if (option) {
  ...
  if (option === null) {
    ...
    const v = coerce(value, option, parsedOptions[option.name]);
    if (v !== undefined) {
      if (parsedOptions[option.name] !== v) {
        ...
        if (option.deprecated !== undefined && option.deprecated !== false) {
          warnings.push(
            `Option ${JSON.stringify(option.name)} is deprecated${
